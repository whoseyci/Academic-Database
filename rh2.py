...
async function copyText(txt,btn){try{await navigator.clipboard.writeText(String(txt||'')); if(btn){const old=btn.textContent; btn.textContent='✓'; btn.classList.add('ok'); setTimeout(()=>{btn.textContent=old; btn.classList.remove('ok')},900)} toast('Copied')}catch(e){prompt('Copy:',txt)}} function copyDecoded(txt,btn){copyText(decodeURIComponent(txt||''),btn)}
...
function sourceSuggestionControls(id){return `<button class="btn small good" onclick="post('/api/accept-source-card',{suggestion_id:'${id}'})">Accept</button><button class="btn small bad" onclick="post('/api/reject-source-card',{suggestion_id:'${id}',label:'too_broad'})">Reject</button>`}
function citationLocationControls(id,c){let val=(c.char_start!=null&&c.char_end!=null)?`${c.char_start}:${c.char_end}`:'';return `<button class="btn small good" onclick="post('/api/verify-location',{suggestion_id:'${id}',status:'accepted'})">Accept loc</button><button class="btn small bad" onclick="post('/api/verify-location',{suggestion_id:'${id}',status:'rejected'})">Reject loc</button><input class="range-input" id="rg-${id}" placeholder="START:END" value="${esc(val)}"><button class="btn small" onclick="post('/api/move-citation-location',{suggestion_id:'${id}',range:$('rg-${id}').value})">Move range</button>`}

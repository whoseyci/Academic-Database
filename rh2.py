...
REVIEW_UI_HTML = r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Review Cockpit</title><style>body{font-family:system-ui;margin:0;background:#0f1115;color:#eef2ff}header{padding:12px 18px;border-bottom:1px solid #333;display:flex;justify-content:space-between}.wrap{display:grid;grid-template-columns:320px 1fr;gap:14px;padding:14px}.side,.card{background:#181b22;border:1px solid #333946;border-radius:16px;padding:14px}.tab,.btn{border:1px solid #4b5563;border-radius:9px;background:#293244;color:white;padding:6px 9px;margin:3px;cursor:pointer}.tab.active{background:#4f75bd}.hidden{display:none}.chip{border:1px solid #555;border-radius:999px;padding:2px 8px;margin:2px;font-size:12px}.item{border-top:1px solid #333;padding:10px 0}.id{font-family:monospace;color:#dbeafe}.cmd,pre{font-family:monospace;background:#090a0d;color:#c7d2fe;padding:6px;border-radius:6px;overflow:auto}.graph-layout{display:grid;grid-template-columns:1fr 360px;gap:12px}canvas{width:100%;height:720px;background:#080b12;border:1px solid #333;border-radius:12px}.graph-side{background:#11151d;border:1px solid #333;border-radius:12px;padding:12px;max-height:720px;overflow:auto}.muted{color:#aab2c5}input,select{background:#0b0d12;color:#eef2ff;border:1px solid #424957;border-radius:7px;padding:6px}.toast{position:fixed;right:12px;bottom:12px;background:#dbeafe;color:#111;padding:9px;border-radius:9px}.iconbtn{background:#111827;border:1px solid #3f4654;border-radius:8px;color:#dbeafe;width:26px;height:26px}.iconbtn.ok{background:#064e3b}@media(max-width:1000px){.wrap,.graph-layout{grid-template-columns:1fr}}</style></head><body><header><div><b>Academic Database Review Cockpit</b><div class="muted">Graph + review hub</div></div><div><button class="btn" onclick="load()">Refresh</button><button class="btn" onclick="post('/api/refresh-signals',{})">Refresh signals</button></div></header><div class="wrap"><aside class="side"><input id="q" placeholder="Filter" oninput="render()" style="width:100%"><p><button class="tab active" data-tab="graph" onclick="setTab('graph')">Graph</button><button class="tab" data-tab="queue" onclick="setTab('queue')">Review</button><button class="tab" data-tab="scs" onclick="setTab('scs')">Source suggestions</button><button class="tab" data-tab="cls" onclick="setTab('cls')">Citation locations</button><button class="tab" data-tab="syn" onclick="setTab('syn')">Synthesis</button></p><div id="stats"></div><p class="muted">Click a dot to focus its neighborhood. Drag empty space to rotate. Wheel zoom. Shift-drag pan.</p></aside><main><section class="card" id="graphCard"><h3>Evidence Graph <button class="btn" onclick="setGraphView('all')">All</button><button class="btn" onclick="setGraphView('sources')">Sources</button><button class="btn" onclick="setGraphView('claims')">Claims</button><button class="btn" onclick="setGraphView('citations')">Citations</button><button class="btn" onclick="setGraphView('focus')">Focus</button><button class="btn" onclick="setGraphMode('2d')">2D</button><button class="btn" onclick="setGraphMode('3d')">3D</button><button class="btn" onclick="toggleRotate()">Auto</button><button class="btn" onclick="resetGraphView()">Reset</button></h3><div class="graph-layout"><div><canvas id="graph" width="1400" height="820"></canvas><p id="graphInfo" class="muted"></p></div><aside id="nodeDetail" class="graph-side"><h3>Node detail</h3><p class="muted">Click a dot.</p></aside></div></section><section class="card hidden" id="queueCard"><h3>Review <span id="queueCount"></span></h3><div id="queue"></div></section><section class="card hidden" id="scsCard"><h3>Source suggestions <span id="scsCount"></span></h3><div id="scs"></div></section><section class="card hidden" id="clsCard"><h3>Citation locations <span id="clsCount"></span></h3><div id="cls"></div></section><section class="card hidden" id="synCard"><h3>Synthesis <span id="synCount"></span></h3><div id="syn"></div></section></main></div><div id="toast" class="toast hidden"></div><script>
let state={data:null,tab:'graph',graph:null,graphMode:'3d',graphView:'all',autoRotate:true,rotX:-.28,rotY:.75,zoom:1,panX:0,panY:0,selected:null,mouseMode:null};const $=id=>document.getElementById(id),esc=s=>String(s??'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));function toast(m){$('toast').textContent=m;$('toast').classList.remove('hidden');setTimeout(()=>$('toast').classList.add('hidden'),900)}async function copyText(t,b){await navigator.clipboard.writeText(String(t||''));if(b){let o=b.textContent;b.textContent='✓';b.classList.add('ok');setTimeout(()=>{b.textContent=o;b.classList.remove('ok')},700)}toast('Copied')}async function post(u,o){let r=await fetch(u,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(o)}),j=await r.json();if(!j.ok)alert(JSON.stringify(j));else toast('Updated');await load()}
function chip(x){return x?`<span class="chip">${esc(x)}</span>`:''}function filter(a){let q=$('q').value.toLowerCase();return !q?a:(a||[]).filter(c=>JSON.stringify(c).toLowerCase().includes(q))}function item(c,t){let id=c.claim_id||c.suggestion_id||c.synthesis_id,body=c.claim||c.text||c.synthesis||c.matched_text||'',cmd=t==='claim'?`python rh2.py review-packet ${id}`:t==='scs'?`python rh2.py accept-source-card-suggestion ${id}`:t==='cls'?`python rh2.py assess-citation-location ${id}`:id;return`<div class="item"><b class="id">${esc(id)}</b> <button class="iconbtn" onclick="copyText('${esc(id)}',this)">⧉</button><button class="iconbtn" onclick="copyText(\`${esc(body)}\`,this)">📋</button>${chip(c.status)}${chip(c.evidence_grade)}${chip(c.centrality_score?'central '+Number(c.centrality_score).toFixed(2):'')}<p>${esc(body)}</p><button class="btn" onclick="copyText(\`${esc(cmd)}\`,this)">Copy command</button><div class="cmd">${esc(cmd)}</div></div>`}
function setTab(t){state.tab=t;document.querySelectorAll('.tab').forEach(b=>b.classList.toggle('active',b.dataset.tab===t));['graph','queue','scs','cls','syn'].forEach(x=>$(x+'Card').classList.toggle('hidden',x!==t));render()}function setGraphView(v){state.graphView=v;state.graph=null;draw(true)}function setGraphMode(m){state.graphMode=m;state.graph=null;draw(true)}function toggleRotate(){state.autoRotate=!state.autoRotate}function resetGraphView(){state.rotX=-.28;state.rotY=.75;state.zoom=1;state.panX=0;state.panY=0;state.selected=null;state.graphView='all';state.graph=null;draw(true);detail()}
function render(){let d=state.data;if(!d)return;$('stats').innerHTML=Object.entries(d.stats).map(([k,v])=>`<div>${esc(k)}: <b>${esc(v)}</b></div>`).join('');let q=filter(d.review_queue),sc=filter(d.source_card_suggestions),cl=filter(d.citation_location_suggestions),sy=filter(d.synthesis_cards);$('queueCount').textContent=q.length;$('scsCount').textContent=sc.length;$('clsCount').textContent=cl.length;$('synCount').textContent=sy.length;$('queue').innerHTML=q.map(x=>item(x,'claim')).join('');$('scs').innerHTML=sc.map(x=>item(x,'scs')).join('');$('cls').innerHTML=cl.map(x=>item(x,'cls')).join('');$('syn').innerHTML=sy.map(x=>item(x,'syn')).join('');if(state.tab==='graph')draw(false)}async function load(){try{state.data=await(await fetch('/api/state?limit=180')).json()}catch(e){state.data=await(await fetch('data/review_export.json')).json()}render();detail()}
function subset(){let g=state.data?.graph||{nodes:[],edges:[]},nodes=g.nodes||[],edges=g.edges||[];if(state.graphView==='sources')nodes=nodes.filter(n=>n.kind==='source');else if(state.graphView==='claims')nodes=nodes.filter(n=>['source','claim'].includes(n.kind));else if(state.graphView==='citations')nodes=nodes.filter(n=>['source','citation','range'].includes(n.kind));else if(state.graphView==='focus'&&state.selected){let keep=new Set([state.selected.id]);for(const e of edges){if(e.source===state.selected.id)keep.add(e.target);if(e.target===state.selected.id)keep.add(e.source)}nodes=nodes.filter(n=>keep.has(n.id))}let ids=new Set(nodes.map(n=>n.id));return{nodes,edges:edges.filter(e=>ids.has(e.source)&&ids.has(e.target))}}function detail(){let n=state.selected,box=$('nodeDetail');if(!n){box.innerHTML='<h3>Node detail</h3><p class="muted">Click a dot.</p>';return}let rel=(state.data?.graph?.edges||[]).filter(e=>e.source===n.id||e.target===n.id).slice(0,30);box.innerHTML=`<h3>${esc(n.kind)} node</h3><div class="id">${esc(n.id)}</div><button class="btn" onclick="copyText('${esc(n.id)}',this)">Copy ID</button><button class="btn" onclick="setGraphView('focus')">Focus neighborhood</button><pre>${esc(JSON.stringify(n,null,2))}</pre><h3>Related edges</h3>${rel.map(e=>`<div class="item">${chip(e.kind)}<div class="muted">${esc(e.source)} → ${esc(e.target)}</div></div>`).join('')}`}
function draw(force){let sub=subset(),canvas=$('graph');if(!canvas)return;let ctx=canvas.getContext('2d'),W=canvas.width,H=canvas.height,colors={source:'#7dd3fc',claim:'#a78bfa',citation:'#fcd34d',range:'#fb7185'};if(force||!state.graph||state.graph.key!==state.graphView){let nodes=sub.nodes.slice(0,520).map((n,i)=>{let a=i*2.399,r=150+310*Math.random();return{...n,x:Math.cos(a)*r,y:(Math.random()-.5)*460,z:Math.sin(a)*r,vx:0,vy:0,vz:0}});state.graph={key:state.graphView,nodes,edges:sub.edges.slice(0,1100),settled:false}}let nodes=state.graph.nodes,edges=state.graph.edges,map=Object.fromEntries(nodes.map(n=>[n.id,n]));for(let k=0;k<(force||!state.graph.settled?80:2);k++){for(const n of nodes){n.vx+=-n.x*.0007;n.vy+=-n.y*.0007;n.vz+=-n.z*.0007}for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){let a=nodes[i],b=nodes[j],dx=a.x-b.x,dy=a.y-b.y,dz=a.z-b.z,d2=dx*dx+dy*dy+dz*dz+140,f=300/d2;a.vx+=dx*f;b.vx-=dx*f;a.vy+=dy*f;b.vy-=dy*f;a.vz+=dz*f;b.vz-=dz*f}for(const e of edges){let a=map[e.source],b=map[e.target];if(!a||!b)continue;let dx=b.x-a.x,dy=b.y-a.y,dz=b.z-a.z,d=Math.sqrt(dx*dx+dy*dy+dz*dz)||1,f=(d-145)*.006;a.vx+=dx/d*f;b.vx-=dx/d*f;a.vy+=dy/d*f;b.vy-=dy/d*f;a.vz+=dz/d*f;b.vz-=dz/d*f}for(const n of nodes){n.vx*=.86;n.vy*=.86;n.vz*=.86;n.x+=n.vx;n.y+=n.vy;n.z+=n.vz}}state.graph.settled=true;if(state.graphMode==='3d'&&state.autoRotate)state.rotY+=.01;function pr(n){if(state.graphMode==='2d')return{x:W/2+state.panX+n.x*state.zoom,y:H/2+state.panY+n.y*state.zoom,s:state.zoom,d:0};let cy=Math.cos(state.rotY),sy=Math.sin(state.rotY),cx=Math.cos(state.rotX),sx=Math.sin(state.rotX),x1=n.x*cy-n.z*sy,z1=n.x*sy+n.z*cy,y1=n.y*cx-z1*sx,z2=n.y*sx+z1*cx,s=680/(680+z2)*state.zoom;return{x:W/2+state.panX+x1*s,y:H/2+state.panY+y1*s,s,d:z2}}for(const n of nodes){let p=pr(n);n.sx=p.x;n.sy=p.y;n.scale=p.s;n.depth=p.d}ctx.clearRect(0,0,W,H);let grad=ctx.createRadialGradient(W/2,H/2,20,W/2,H/2,900);grad.addColorStop(0,'#263658');grad.addColorStop(1,'#05070c');ctx.fillStyle=grad;ctx.fillRect(0,0,W,H);for(const e of edges){let a=map[e.source],b=map[e.target];if(!a||!b)continue;let sel=state.selected&&(e.source===state.selected.id||e.target===state.selected.id);ctx.strokeStyle=sel?'rgba(251,214,101,.8)':'rgba(180,195,230,.22)';ctx.lineWidth=sel?2.6:1;ctx.beginPath();ctx.moveTo(a.sx,a.sy);ctx.lineTo(b.sx,b.sy);ctx.stroke();if(state.graphView==='focus'||sel){ctx.fillStyle='#cbd5e1';ctx.font='11px system-ui';ctx.fillText(e.kind,(a.sx+b.sx)/2,(a.sy+b.sy)/2)}}for(const n of nodes.sort((a,b)=>a.depth-b.depth)){let base=n.kind==='source'?12:n.kind==='claim'?5:n.kind==='citation'?6:8,r=base*Math.max(.45,Math.min(2,n.scale));n.r=r;let sel=state.selected&&state.selected.id===n.id;ctx.fillStyle=sel?'#fcd34d':(colors[n.kind]||'#94a3b8');ctx.beginPath();ctx.arc(n.sx,n.sy,sel?r+4:r,0,Math.PI*2);ctx.fill();ctx.strokeStyle=sel?'white':'rgba(255,255,255,.2)';ctx.stroke();if(sel||['source','range'].includes(n.kind)){ctx.fillStyle='#dbeafe';ctx.font='12px system-ui';ctx.fillText(String(n.label||n.id).slice(0,34),n.sx+r+4,n.sy+4)}}$('graphInfo').textContent=`${nodes.length} nodes · ${edges.length} edges · ${state.graphMode.toUpperCase()} · ${state.graphView} view`;function pt(ev){let r=canvas.getBoundingClientRect();return{x:(ev.clientX-r.left)*W/r.width,y:(ev.clientY-r.top)*H/r.height}}canvas.onmousedown=ev=>{let p=pt(ev);state.lastX=p.x;state.lastY=p.y;state.downX=p.x;state.downY=p.y;let hit=nodes.find(n=>(n.sx-p.x)**2+(n.sy-p.y)**2<(Math.max(12,n.r)+7)**2);if(hit&&ev.altKey){state.drag=hit;state.mouseMode='node'}else if(ev.shiftKey||ev.button===2){state.mouseMode='pan'}else{state.mouseMode='rotate';state.autoRotate=false}};canvas.onmousemove=ev=>{let p=pt(ev),dx=p.x-state.lastX,dy=p.y-state.lastY;if(state.mouseMode==='rotate'){state.rotY+=dx*.006;state.rotX=Math.max(-1.45,Math.min(1.45,state.rotX+dy*.006));draw(false)}else if(state.mouseMode==='pan'){state.panX+=dx;state.panY+=dy;draw(false)}else if(state.mouseMode==='node'&&state.drag){state.drag.x+=dx/state.zoom;state.drag.y+=dy/state.zoom;draw(false)}state.lastX=p.x;state.lastY=p.y};canvas.onmouseup=ev=>{let p=pt(ev),moved=Math.hypot(p.x-state.downX,p.y-state.downY);if(moved<4){let hit=nodes.find(n=>(n.sx-p.x)**2+(n.sy-p.y)**2<(Math.max(12,n.r)+7)**2);state.selected=hit||null;if(hit){state.graphView='focus';state.graph=null;state.zoom=1.35;toast('Focused '+(hit.label||hit.id))}draw(false);detail()}state.drag=null;state.mouseMode=null};canvas.onwheel=ev=>{ev.preventDefault();state.zoom*=ev.deltaY<0?1.1:.9;state.zoom=Math.max(.25,Math.min(4,state.zoom));draw(false)}}setInterval(()=>{if(state.tab==='graph'&&state.graphMode==='3d'&&state.autoRotate)draw(false)},70);window.addEventListener('keydown',e=>{let tag=(e.target&&e.target.tagName||'').toLowerCase();if(['input','textarea','select'].includes(tag))return;if(e.key==='0')setTab('graph');if(e.key==='1')setTab('queue');if(e.key==='2')setTab('scs');if(e.key==='3')setTab('cls');if(e.key==='4')setTab('syn')});load();</script></body></html>
'''

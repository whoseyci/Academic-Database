...
function initGraph(force){let g=state.data?.graph;if(!g)return; const canvas=$('graph'); if(!canvas)return; const ctx=canvas.getContext('2d'); const W=canvas.width,H=canvas.height; const colors={source:'#7dd3fc',claim:'#a78bfa',citation:'#fcd34d',range:'#fb7185'}; if(force||!state.graph){let nodes=(g.nodes||[]).slice(0,360).map((n,i)=>({...n,x:W/2+Math.cos(i*2.4)*200*Math.random(),y:H/2+Math.sin(i*1.7)*180*Math.random(),vx:0,vy:0})); state.graph={nodes,edges:(g.edges||[]).slice(0,720)} } let nodes=state.graph.nodes, edges=state.graph.edges, map=Object.fromEntries(nodes.map(n=>[n.id,n])); for(let step=0;step<60;step++){for(const n of nodes){n.vx+=(W/2-n.x)*.0009;n.vy+=(H/2-n.y)*.0009} for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){let a=nodes[i],b=nodes[j],dx=a.x-b.x,dy=a.y-b.y,d2=dx*dx+dy*dy+80,f=220/d2;a.vx+=dx*f;b.vx-=dx*f;a.vy+=dy*f;b.vy-=dy*f} for(const e of edges){let a=map[e.source],b=map[e.target]; if(!a||!b)continue; let dx=b.x-a.x,dy=b.y-a.y,d=Math.sqrt(dx*dx+dy*dy)||1,target=100+(e.weight||1)*5,f=(d-target)*.006;a.vx+=dx/d*f;b.vx-=dx/d*f;a.vy+=dy/d*f;b.vy-=dy/d*f} for(const n of nodes){n.vx*=.86;n.vy*=.86;n.x=Math.max(20,Math.min(W-20,n.x+n.vx));n.y=Math.max(20,Math.min(H-20,n.y+n.vy))}} ctx.clearRect(0,0,W,H); for(const e of edges){let a=map[e.source],b=map[e.target]; if(!a||!b)continue;ctx.strokeStyle='rgba(180,190,220,.22)';ctx.lineWidth=Math.min(4,.6+(e.weight||1)*.25);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke()} for(const n of nodes){let r=n.kind==='source'?10:n.kind==='claim'?5:7;ctx.fillStyle=colors[n.kind]||'#94a3b8';ctx.beginPath();ctx.arc(n.x,n.y,r,0,Math.PI*2);ctx.fill(); if(['source','range'].includes(n.kind)){ctx.fillStyle='#dbeafe';ctx.font='12px system-ui';ctx.fillText(String(n.label||n.id).slice(0,28),n.x+r+3,n.y+4)}} $('graphInfo').textContent=`${nodes.length} nodes · ${edges.length} edges`; canvas.onmousedown=ev=>{let rect=canvas.getBoundingClientRect(),x=(ev.clientX-rect.left)*W/rect.width,y=(ev.clientY-rect.top)*H/rect.height;state.drag=nodes.find(n=>(n.x-x)**2+(n.y-y)**2<180)}; canvas.onmousemove=ev=>{if(!state.drag)return;let rect=canvas.getBoundingClientRect();state.drag.x=(ev.clientX-rect.left)*W/rect.width;state.drag.y=(ev.clientY-rect.top)*H/rect.height;initGraph(false)}; canvas.onmouseup=()=>state.drag=null; canvas.onmouseleave=()=>state.drag=null}

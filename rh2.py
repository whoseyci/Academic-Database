...
function initGraph(force){
  ...
  if(force||!state.graph){let nodes=(g.nodes||[]).slice(0,420).map((n,i)=>{let a=i*2.399,r=120+260*Math.random();return {...n,x:Math.cos(a)*r,y:(Math.random()-.5)*420,z:Math.sin(a)*r,vx:0,vy:0,vz:0}}); state.graph={nodes,edges:(g.edges||[]).slice(0,900),settled:false};}
  let nodes=state.graph.nodes, edges=state.graph.edges, map=Object.fromEntries(nodes.map(n=>[n.id,n])); let steps=force||!state.graph.settled?80:3;
  for(let step=0;step<steps;step++){
    for(const n of nodes){n.vx+=(-n.x)*0.0008;n.vy+=(-n.y)*0.0008;n.vz+=(-n.z)*0.0008}
    for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){let a=nodes[i],b=nodes[j],dx=a.x-b.x,dy=a.y-b.y,dz=a.z-b.z,d2=dx*dx+dy*dy+dz*dz+120,f=260/d2;a.vx+=dx*f;b.vx-=dx*f;a.vy+=dy*f;b.vy-=dy*f;a.vz+=dz*f;b.vz-=dz*f}
    for(const e of edges){let a=map[e.source],b=map[e.target]; if(!a||!b)continue; let dx=b.x-a.x,dy=b.y-a.y,dz=b.z-a.z,d=Math.sqrt(dx*dx+dy*dy+dz*dz)||1,target=130+(e.weight||1)*8,f=(d-target)*0.006;a.vx+=dx/d*f;b.vx-=dx/d*f;a.vy+=dy/d*f;b.vy-=dy/d*f;a.vz+=dz/d*f;b.vz-=dz/d*f}
    for(const n of nodes){n.vx*=.86;n.vy*=.86;n.vz*=.86;n.x+=n.vx;n.y+=n.vy;n.z+=n.vz}
  }
  state.graph.settled=true; if(state.graphMode==='3d'&&state.autoRotate)state.angle+=0.012;
  function project(n){if(state.graphMode==='2d')return {x:W/2+n.x,y:H/2+n.y,scale:1,depth:0}; let ca=Math.cos(state.angle),sa=Math.sin(state.angle),rx=n.x*ca-n.z*sa,rz=n.x*sa+n.z*ca,scale=560/(560+rz);return {x:W/2+rx*scale,y:H/2+n.y*scale,scale,depth:rz}}
  for(const n of nodes){let p=project(n);n.sx=p.x;n.sy=p.y;n.depth=p.depth;n.scale=p.scale}
  ctx.clearRect(0,0,W,H); let grad=ctx.createRadialGradient(W/2,H/2,20,W/2,H/2,Math.max(W,H)/1.5);grad.addColorStop(0,'rgba(38,54,88,.35)');grad.addColorStop(1,'rgba(5,7,12,1)');ctx.fillStyle=grad;ctx.fillRect(0,0,W,H);
  ...
  for(const e of sortedEdges){let a=map[e.source],b=map[e.target]; if(!a||!b)continue;let alpha=state.graphMode==='3d'?Math.max(.08,Math.min(.42,(a.scale+b.scale)/5)):.22;ctx.strokeStyle=`rgba(180,195,230,${alpha})`;ctx.lineWidth=Math.min(4,.5+(e.weight||1)*.25*((a.scale+b.scale)/2));ctx.beginPath();ctx.moveTo(a.sx,a.sy);ctx.lineTo(b.sx,b.sy);ctx.stroke()}
  let sorted=nodes.slice().sort((a,b)=>a.depth-b.depth);
  for(const n of sorted){let base=n.kind==='source'?10:n.kind==='claim'?5:n.kind==='citation'?6:7,r=base*(state.graphMode==='3d'?Math.max(.45,Math.min(1.8,n.scale)):1);ctx.fillStyle=colors[n.kind]||'#94a3b8';ctx.globalAlpha=state.graphMode==='3d'?Math.max(.45,Math.min(1,n.scale)):.95;ctx.beginPath();ctx.arc(n.sx,n.sy,r,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;if(['source','range'].includes(n.kind)||r>8){ctx.fillStyle='#dbeafe';ctx.font=`${Math.max(10,12*(state.graphMode==='3d'?n.scale:1))}px system-ui`;ctx.fillText(String(n.label||n.id).slice(0,34),n.sx+r+3,n.sy+4)}}
  $('graphInfo').textContent=`${nodes.length} nodes · ${edges.length} edges · ${state.graphMode.toUpperCase()} ${state.autoRotate?'· rotating':''}`;
  canvas.onmousedown=ev=>{let rect=canvas.getBoundingClientRect(),x=(ev.clientX-rect.left)*W/rect.width,y=(ev.clientY-rect.top)*H/rect.height;state.drag=nodes.find(n=>(n.sx-x)**2+(n.sy-y)**2<220)};
  canvas.onmousemove=ev=>{if(!state.drag)return;let rect=canvas.getBoundingClientRect();state.drag.x=(ev.clientX-rect.left)*W/rect.width-W/2;state.drag.y=(ev.clientY-rect.top)*H/rect.height-H/2;initGraph(false)}; canvas.onmouseup=()=>state.drag=null; canvas.onmouseleave=()=>state.drag=null;
}
setInterval(()=>{if(state.tab==='graph'&&state.graphMode==='3d'&&state.autoRotate) initGraph(false)},90);
window.addEventListener('keydown',e=>{if((e.metaKey||e.ctrlKey)&&e.key==='r'){e.preventDefault();load()} if(e.key==='0')setTab('graph'); if(e.key==='1')setTab('queue'); if(e.key==='2')setTab('scs'); if(e.key==='3')setTab('cls'); if(e.key==='4')setTab('syn')}); load();
</script></body></html>'''

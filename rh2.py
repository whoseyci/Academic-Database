...
:root{--bg:#0f1115;--panel:#181b22;--panel2:#20242d;--line:#333946;--text:#eef2ff;--muted:#aab2c5;--good:#6ee7b7;--warn:#f6d365;--bad:#fb7185;--blue:#93c5fd}*{box-sizing:border-box}body{font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;margin:0;background:var(--bg);color:var(--text)}header{padding:14px 18px;border-bottom:1px solid var(--line);position:sticky;top:0;background:rgba(15,17,21,.94);backdrop-filter:blur(10px);z-index:5;display:flex;align-items:center;gap:14px;justify-content:space-between}h1{font-size:20px;margin:0}h2{font-size:16px;margin:0 0 10px}.muted{color:var(--muted)}.wrap{display:grid;grid-template-columns:300px 1fr;gap:14px;padding:14px}.side{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:12px;height:calc(100vh - 78px);overflow:auto;position:sticky;top:78px}.main{display:grid;gap:14px}.card{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:14px;box-shadow:0 10px 30px rgba(0,0,0,.18)}.cols{display:grid;grid-template-columns:1fr 1fr;gap:14px}.item{border:1px solid transparent;border-top-color:var(--line);padding:12px 0}.item:hover{background:rgba(255,255,255,.025)}.item-title{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.id{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;color:#dbeafe}.chip{display:inline-flex;border:1px solid #4b5563;border-radius:999px;padding:2px 8px;margin:2px;font-size:12px;color:#dbe4ff}.good{border-color:#167c59;color:var(--good)}.warn{border-color:#8a6d16;color:var(--warn)}.bad{border-color:#8f2745;color:var(--bad)}.blue{border-color:#2563eb;color:var(--blue)}.btn{background:#293244;color:#fff;border:1px solid #4b5563;border-radius:9px;padding:7px 10px;margin:3px;cursor:pointer}.btn:hover{background:#344058}.btn.good{background:#064e3b}.btn.bad{background:#5f1228}.btn.small{font-size:12px;padding:4px 7px}.toolbar{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.tabs{display:flex;gap:6px;flex-wrap:wrap;margin:10px 0}.tab{border:1px solid var(--line);border-radius:999px;background:#11151c;color:var(--muted);padding:6px 10px;cursor:pointer}.tab.active{background:#243149;color:#fff;border-color:#4f75bd}input,select,textarea{background:#0b0d12;color:var(--text);border:1px solid #424957;border-radius:9px;padding:8px;width:100%;box-sizing:border-box}.row{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center}.cmd{font-family:ui-monospace,monospace;background:#090a0d;padding:6px;border-radius:7px;display:inline-block;color:#c7d2fe}.text{line-height:1.45}.hidden{display:none}.toast{position:fixed;right:16px;bottom:16px;background:#dbeafe;color:#111827;border-radius:12px;padding:10px 14px;box-shadow:0 8px 30px rgba(0,0,0,.35);z-index:20}.stat{display:flex;justify-content:space-between;border-bottom:1px solid #2d3340;padding:5px 0}.section-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:8px}@media(max-width:900px){.wrap{grid-template-columns:1fr}.side{position:relative;top:auto;height:auto}.cols{grid-template-columns:1fr}}
</style></head><body><header><div><h1>Academic Database Review Cockpit</h1><div class="muted">Interactive local review + suggestion triage</div></div><div class="toolbar"><button class="btn" onclick="load()">Refresh</button><button class="btn" onclick="post('/api/refresh-signals',{})">Refresh signals</button><button class="btn" onclick="copyPageSummary()">Copy summary</button></div></header><div class="wrap"><aside class="side"><h2>Controls</h2><input id="q" placeholder="Filter visible cards…" oninput="render()"><div class="tabs"><button class="tab active" data-tab="queue" onclick="setTab('queue')">Review</button><button class="tab" data-tab="scs" onclick="setTab('scs')">Source suggestions</button><button class="tab" data-tab="cls" onclick="setTab('cls')">Citation locations</button><button class="tab" data-tab="syn" onclick="setTab('syn')">Synthesis</button></div><div id="stats"></div><p class="muted">Tip: ⌘/Ctrl+C works normally. Each card also has copy buttons for IDs and commands.</p></aside><main class="main"><section class="card" id="queueCard"><div class="section-head"><h2>High-value review queue</h2><span id="queueCount" class="muted"></span></div><div id="queue"></div></section><section class="card hidden" id="scsCard"><div class="section-head"><h2>Source-card suggestions</h2><span id="scsCount" class="muted"></span></div><div id="scs"></div></section><section class="card hidden" id="clsCard"><div class="section-head"><h2>Citation-location suggestions</h2><span id="clsCount" class="muted"></span></div><div id="cls"></div></section><section class="card hidden" id="synCard"><div class="section-head"><h2>Synthesis cards</h2><span id="synCount" class="muted"></span></div><div id="syn"></div></section></main></div><div id="toast" class="toast hidden"></div><script>
let state={data:null,tab:'queue'}; const $=id=>document.getElementById(id); const esc=s=>String(s??'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));
function toast(msg){$('toast').textContent=msg;$('toast').classList.remove('hidden');setTimeout(()=>$('toast').classList.add('hidden'),1400)}
async function copyText(txt){try{await navigator.clipboard.writeText(String(txt||''));toast('Copied')}catch(e){prompt('Copy:',txt)}}
...
function chip(x,cls=''){return x?`<span class="chip ${cls}">${esc(x)}</span>`:''} function statusCls(s){return s==='verified'?'good':s==='rejected'||s==='superseded'?'bad':'warn'}
function hay(c){return [c.claim_id,c.suggestion_id,c.synthesis_id,c.status,c.evidence_grade,c.claim,c.text,c.synthesis,c.matched_text,c.source_id,c.citation_hint,c.suggested_card_role,c.suggested_claim_type].join(' ').toLowerCase()}
function filter(arr){let q=$('q').value.toLowerCase().trim(); return !q?arr:(arr||[]).filter(c=>hay(c).includes(q))}
...
function item(c,type){let id=c.claim_id||c.suggestion_id||c.synthesis_id; let body=c.claim||c.text||c.synthesis||c.matched_text||''; let cmd=commandFor(c,type); return `<article class="item"><div class="item-title"><b class="id">${esc(id)}</b>${chip(c.status,statusCls(c.status))}${chip(c.evidence_grade,'blue')}${chip(c.suggested_card_role||c.card_role,'blue')}${c.centrality_score?chip('central '+c.centrality_score,'blue'):''}${c.score?chip('score '+c.score):''}</div><p class="text">${esc(body)}</p><div class="toolbar"><button class="btn small" onclick="copyText('${esc(id)}')">Copy ID</button><button class="btn small" onclick="copyText(\`${esc(cmd)}\`)">Copy command</button>${type==='claim'?reviewControls(id):''}${type==='scs'?`<button class="btn small good" onclick="post('/api/accept-source-card',{suggestion_id:'${id}'})">Accept</button><button class="btn small bad" onclick="post('/api/reject-source-card',{suggestion_id:'${id}',label:'too_broad'})">Reject</button>`:''}${type==='cls'?`<button class="btn small good" onclick="post('/api/verify-location',{suggestion_id:'${id}',status:'accepted'})">Accept loc</button><button class="btn small bad" onclick="post('/api/verify-location',{suggestion_id:'${id}',status:'rejected'})">Reject loc</button>`:''}</div><div class="cmd">${esc(cmd)}</div></article>`}
function reviewControls(id){return `<select id="st-${id}" style="width:180px"><option>verified</option><option>candidate_needs_review</option><option>needs_page_check</option><option>needs_source_check</option><option>rejected</option><option>superseded</option></select><input id="nt-${id}" placeholder="review note" style="width:260px"><button class="btn small good" onclick="post('/api/review',{claim_id:'${id}',status:$('st-${id}').value,note:$('nt-${id}').value,label:['good_claim']})">Review</button>`}
function setTab(t){state.tab=t; document.querySelectorAll('.tab').forEach(b=>b.classList.toggle('active',b.dataset.tab===t)); ['queue','scs','cls','syn'].forEach(x=>$(x+'Card').classList.toggle('hidden',x!==t)); render()}
function render(){let d=state.data;if(!d)return; $('stats').innerHTML='<h2>Stats</h2>'+Object.entries(d.stats).map(([k,v])=>`<div class="stat"><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join(''); let q=filter(d.review_queue); let scs=filter(d.source_card_suggestions); let cls=filter(d.citation_location_suggestions); let syn=filter(d.synthesis_cards); $('queueCount').textContent=q.length; $('scsCount').textContent=scs.length; $('clsCount').textContent=cls.length; $('synCount').textContent=syn.length; $('queue').innerHTML=q.map(x=>item(x,'claim')).join('')||'<p class="muted">No items.</p>'; $('scs').innerHTML=scs.map(x=>item(x,'scs')).join('')||'<p class="muted">No suggestions.</p>'; $('cls').innerHTML=cls.map(x=>item(x,'cls')).join('')||'<p class="muted">No citation locations.</p>'; $('syn').innerHTML=syn.map(x=>item(x,'syn')).join('')||'<p class="muted">No synthesis cards.</p>'}
async function load(){state.data=await (await fetch('/api/state?limit=180')).json(); render()} function copyPageSummary(){let d=state.data;if(!d)return; copyText(`Academic Database Review Summary\n${JSON.stringify(d.stats,null,2)}`)}
window.addEventListener('keydown',e=>{if((e.metaKey||e.ctrlKey)&&e.key==='r'){e.preventDefault();load()} if(e.key==='1')setTab('queue'); if(e.key==='2')setTab('scs'); if(e.key==='3')setTab('cls'); if(e.key==='4')setTab('syn')}); load();
</script></body></html>'''

...
let state={data:null,tab:'graph',graph:null,drag:null,graphMode:'3d',autoRotate:true,rotX:-0.28,rotY:0.75,zoom:1,panX:0,panY:0,hover:null,mouseMode:null,lastX:0,lastY:0}; const $=id=>document.getElementById(id); const esc=s=>String(s??'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));
...
function initGraph(force){
  ...
  $('graphInfo').textContent=`${nodes.length} nodes · ${edges.length} edges · ${state.graphMode.toUpperCase()} · drag rotate · wheel zoom · shift-drag pan ${state.autoRotate?'· auto-rotating':''}`;
  ...
  canvas.onmousedown=ev=>{let p=pointer(ev);state.lastX=p.x;state.lastY=p.y;let hit=nodes.find(n=>(n.sx-p.x)**2+(n.sy-p.y)**2<(Math.max(12,n.r)+7)**2); if(hit&&ev.altKey){state.drag=hit;state.mouseMode='node'} else if(ev.shiftKey||ev.button===2){state.mouseMode='pan'} else {state.mouseMode='rotate';state.autoRotate=false}};
  ...
  canvas.onmouseup=()=>{state.drag=null;state.mouseMode=null}; canvas.onmouseleave=()=>{state.drag=null;state.mouseMode=null}; canvas.oncontextmenu=e=>e.preventDefault(); canvas.onwheel=ev=>{ev.preventDefault();state.zoom*=ev.deltaY<0?1.1:.9;state.zoom=Math.max(.25,Math.min(4,state.zoom));initGraph(false)};
}
